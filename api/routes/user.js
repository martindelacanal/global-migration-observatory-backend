...
router.get('/article', async (req, res) => {
  try {
    ...
    // Get articles with preview images
    const query = `
      SELECT 
        a.id,
        a.title_${lang === 'en' ? 'en' : 'es'} as title,
        a.subtitle_${lang === 'en' ? 'en' : 'es'} as subtitle,
        a.content_${lang === 'en' ? 'en' : 'es'} as content,
        a.author,
        a.publication_date as date,
        a.category_id as categoryId,
        a.priority,
        a.article_status_id,
        a.slug_${lang === 'en' ? 'en' : 'es'} as slug,
        a.view_count,
        a.featured,
        a.creation_date as createdAt,
        a.modification_date as updatedAt,
    ...
    const formattedArticles = await Promise.all(articles.map(async (article) => {
      ...
      let contentWithUrls = article.content;
      if (contentWithUrls) {
        contentWithUrls = await replaceS3KeysWithSignedUrls(contentWithUrls);
      }

      return {
        id: article.id,
        titleEnglish: lang === 'en' ? article.title : null,
        titleSpanish: lang === 'es' ? article.title : null,
        subtitleEnglish: lang === 'en' ? article.subtitle : null,
        subtitleSpanish: lang === 'es' ? article.subtitle : null,
        contentEnglish: lang === 'en' ? contentWithUrls : null,
        contentSpanish: lang === 'es' ? contentWithUrls : null,
